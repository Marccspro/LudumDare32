...
public class Player extends Entity {
	...
	public List<PotatoItem> potatoes;
	...
	public Player(float x, float y) {
		...
		potatoes = new ArrayList<PotatoItem>();
		...
		if (!Inventory.showInv) {
			...
			if (Keyboard.isKeyDown(Keyboard.KEY_G)) {
				if (!throwP) {
					throwP = true;
					if (potatoes.size() > 0) {
						Game.getGame().getLevel().add(new PotatoGrenade(x, y, 0.3f, xd, yd));
						potatoes.remove(0);
					}
				}else {
					throwTimer++;
	...
	public void addPotion(PotionItem potion) {
		inv.add(potion);
	}
	
	public void addPotato(PotatoItem potato) {
		potatoes.add(potato);
	}
	
	public List<PotatoItem> getPotatoes() {
		return potatoes;
	}
