...
import fr.veridiangames.main.game.entities.mobs.DoubleHeadMob;
...
import fr.veridiangames.main.game.level.tiles.BlockTile;
import fr.veridiangames.main.game.level.tiles.BrickTile;
import fr.veridiangames.main.game.level.tiles.DirtTile;
import fr.veridiangames.main.game.level.tiles.DoorTile;
import fr.veridiangames.main.game.level.tiles.EndDoorTile;
import fr.veridiangames.main.game.level.tiles.GrassTile;
import fr.veridiangames.main.game.level.tiles.RockTile;
import fr.veridiangames.main.game.level.tiles.StoneBrickTile;
import fr.veridiangames.main.game.level.tiles.StoneTile;
import fr.veridiangames.main.game.level.tiles.Tile;
import fr.veridiangames.main.game.level.tiles.UpTile;
import fr.veridiangames.main.game.level.tiles.items.Item;
import fr.veridiangames.main.game.level.tiles.items.KeyItem;
import fr.veridiangames.main.game.level.tiles.items.PotatoItem;
...
import fr.veridiangames.main.game.level.tiles.items.weapons.PingPongItem;
...
import fr.veridiangames.main.game.level.tiles.items.weapons.WaterBottleItem;
...
public class Level extends EntityManager {
	...
	public List<DoorTile> doors = new ArrayList<DoorTile>();
	...
	public Level(String level, Player player) {
		...
		for (int x = 0; x < w; x++) {
			for (int y = 0; y < h; y++) {
				...
				//Tiles
				if (pixels[i] == 0xff6B9941) tiles[x][y] = new GrassTile(x, y);
				else if (pixels[i] == 0xff808080) tiles[x][y] = new RockTile(x, y);
				else if (pixels[i] == 0xffAF4733) tiles[x][y] = new BrickTile(x, y);
				
				else if (pixels[i] == 0xff404040) tiles[x][y] = new StoneBrickTile(x, y);
				else if (pixels[i] == 0xffADADAD) tiles[x][y] = new StoneTile(x, y);
				
				else if (DoorTile.doorColor(pixels[i])) {tiles[x][y] = new DoorTile(x, y, pixels[i], this); doors.add(new DoorTile(x, y, pixels[i], this)); }
				else if (levelDoor(pixels[i])) {tiles[x][y] = new EndDoorTile(x, y, pixels[i]); endPoint[0] = x; endPoint[1] = y; }
				...
				else if (pixels[i] == 0xff00FFFF) {items.add( new PotionItem(x, y)); tiles[x][y] = tiles[x - 1][y];}
				else if (KeyItem.keyColor(pixels[i])) {items.add( new KeyItem(x, y, pixels[i])); tiles[x][y] = tiles[x - 1][y];}
				else if (pixels[i] == 0xffFFD800) {items.add( new PotatoItem(x, y)); tiles[x][y] = tiles[x - 1][y];}
				
				
				//Spawns
				else if (pixels[i] == 0xffFFAA7F) {spawnMob(2, new WormMob(x, y)); tiles[x][y] = tiles[x - 1][y];}
				else if (pixels[i] == 0xffC2B091) {spawnMob(2, new MillipedeMob(x, y)); tiles[x][y] = tiles[x - 1][y];}
				else if (pixels[i] == 0xff624138) {spawnMob(2, new Weird1Mob(x, y)); tiles[x][y] = tiles[x - 1][y];}
				else if (pixels[i] == 0xff7A5246) {spawnMob(2, new DoubleHeadMob(x, y)); tiles[x][y] = tiles[x - 1][y];}
				else if (pixels[i] == 0xff543830) {spawnMob(2, new BossMob(x, y)); tiles[x][y] = tiles[x - 1][y];}
				
				//Others
				else if (pixels[i] == 0xffFFFF00) tiles[x][y] = tiles[x - 1][y];
				else tiles[x][y] = new DirtTile(x, y);
			}	
		}
	...
	private void compile() {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				if (!tile.block) {
					if (right.block) {
						Renderer.wallData(x + 1, y, x + 1, y + 1, ((BlockTile)right).wallTex);
					}
					if (down.block) {
						Renderer.wallData(x + 1, y + 1, x, y + 1, ((BlockTile)down).wallTex);
					}
				}else {
					Renderer.ceilData(x, 1, y, tile.tex);
					if (!right.block) {
						Renderer.wallData(x + 1, y + 1, x + 1, y, ((BlockTile)tile).wallTex);
					}
					if (!down.block) {
						Renderer.wallData(x, y + 1, x + 1, y + 1, ((BlockTile)tile).wallTex);
					}
				}
